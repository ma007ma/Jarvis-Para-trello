...
async function getPanelUrlWithContext(t: TrelloModalContext) {
  ...
  const params = new URLSearchParams({ panel: 'lab', v: 'lab-reactor-20260709-card-context-fix3' });
...
async function createLabCard(t: TrelloModalContext) {
  try {
    ...
    const url = `./lab.html?panel=lab&mode=create&boardId=${encodeURIComponent(boardId)}&cardId=${encodeURIComponent(card.id)}&cardName=${encodeURIComponent(card.name)}&v=lab-reactor-20260709-card-context-fix3`;
    await t.modal({
      title: 'Lab Reactor',
      url: getSignedUrl(t, url),
      height: 920,
...
async function openBoardCreatePanel(t: TrelloModalContext) {
  try {
    ...
    const boardId = board.id ?? '';
    const url = `./lab.html?panel=lab&mode=board-create&boardId=${encodeURIComponent(boardId)}&v=lab-reactor-20260709-card-context-fix3`;
    await t.modal({
